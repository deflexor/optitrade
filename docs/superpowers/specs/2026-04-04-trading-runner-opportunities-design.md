...
**Scope:** `src/internal/dashboard` (BFF + SQLite), new runner package(s) under `src/internal/`, `web/` (routes, header, Opportunities page, settings)  
**Date:** 2026-04-04

## Summary

Add a **per-operator trading runner** started as **goroutine(s) inside the dashboard process**: it scans for **strategy opportunities**, keeps them **fresh for the UI**, respects **risk limits** (e.g. max loss as % of equity), and **auto-enters** only when **bot mode is `auto`**. Operators control **bot mode** (`manual` / `auto` / `paused`) from the **header**; **admins** can **disable** an account (`account_status`) so **no runner** starts. Replace the current **exchange positions list** with an **Opportunities** page that shows **live candidates** and **bot-managed positions** (open / opening / partial) with **Open**, **Cancel**, and **Close** actions. When **paused**, the Opportunities page shows a **Paused** banner and **no live list** (empty body + short copy to resume).
...
1. **Discovery loop** — Periodically evaluate the options universe (per existing strategy/risk building blocks), produce **ranked opportunities** with: strategy name, legs with **bid/ask** (live), **greeks**, **max profit / max loss**, **recommendation** (open / pass) and **short rationale**.
...
- Wire **`internal/risk`** and **`internal/strategy`** into the runner’s evaluate step; exact candidate schema in implementation plan.
...
After this spec is approved in the repo, use **writing-plans** for phased implementation: schema + settings fields, RunnerManager skeleton, minimal opportunity mock → real evaluator, SSE/poll, Opportunities page, execution hooks, retire old positions routes, e2e refresh.
