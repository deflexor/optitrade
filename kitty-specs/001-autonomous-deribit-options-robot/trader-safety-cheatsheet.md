...
Work through with `contracts/config-policy.schema.json` aligned policy (paths relative to repo root: `kitty-specs/001-autonomous-deribit-options-robot/contracts/`).
...
- Exchange API reference (external): see `research/source-register.csv` for curated links
