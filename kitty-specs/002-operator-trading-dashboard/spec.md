...
- Q: For the single primary classification the bot uses (regime vs market mood wording), what should v1 standardize on in the UI? -> A: **Both words always**: paired labels **Regime** (primary) and **Market mood** (secondary) with the **same** canonical value in each line, matching the bot's published classification (or paired unknown/stale when data is bad).
...
8. **Given** a signed-in operator and a snapshot whose timestamp is more than **5 seconds** old relative to the agreed time source, **when** they view the summary strip, **then** they see an explicit stale or refreshing indicator for health, connection mode, equity, and regime/mood fields rather than silent presentation as fresh.

---

### User Story 2 - Portfolio snapshot and market context (Priority: P2)

An operator wants total account balance, a profit and loss view over time, and **Regime** plus **Market mood** labels (paired, same value) reflecting the bot's current classification so they can interpret positions in context and stay aligned with internal docs.

**Why this priority**: Supports supervision and aligns human understanding with automated behavior.

**Independent Test**: Compare dashboard balance and P/L series to exchange-reported or internally reconciled figures; verify **Regime** and **Market mood** values match the engine's single published classification for the same timestamp.
...
3. **Given** the classifier produces a single canonical label, **when** the operator views the dashboard, **then** they see **Regime** (primary) and **Market mood** (secondary) with identical values matching that label, or paired unknown/stale when data is not trustworthy.
...
- **FR-005**: The dashboard MUST display the bot's current classification using **both** operator-visible labels **Regime** (primary) and **Market mood** (secondary), with the **same** value on both lines at the same timestamp. If classification is unavailable or stale, both lines MUST degrade consistently (for example unknown or stale) without contradicting each other.
...
- **FR-019**: The UI MUST treat the **summary snapshot** (uptime, memory, Deribit connection and environment mode, equity, paired **Regime** / **Market mood** values) as **stale** when its bundled timestamp is more than **5 seconds** behind the current time on the agreed reference clock (normally server time supplied with the payload). Stale summaries MUST show a clear stale or refreshing state and MUST NOT be labeled or styled as live-current without that qualification.
...
- **Dashboard snapshot**: Timestamped bundle of health metrics, connection mode, **equity** (primary balance), **Regime** and **Market mood** (paired strings, must match), and freshness flags; **staleness threshold 5 seconds** on the reference clock for the summary strip.
...
- **SC-009**: In acceptance sampling, **Regime** and **Market mood** are always both present and always carry the **same** value when classification is good; mismatch or single-line-only presentation is **zero** tolerated.
