...
## Merge preflight and planned WPs

spec-kitty 1.0.3 may refuse merge when **future** work packages (lane `planned`) have no
`.worktrees/...-WP##` yet. This repo patches that behavior: **missing worktrees are OK for
lane `planned`** (worktrees only exist after `spec-kitty agent workflow implement`).

If `uv tool upgrade spec-kitty-cli` overwrites the fix, re-apply:
